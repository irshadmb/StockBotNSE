		...
		request('https://westus.api.cognitive.microsoft.com/luis/v2.0/apps/748305f4-4a1d-4784-a182-ca3ecfa6ae09?subscription-key=ee68d589970542afae8ddf76b38afd4a&verbose=true&timezoneOffset=0&q='+encodeURI(session.message.text), function (error, response, body) {
		  if (!error && response.statusCode == 200) {
			...
			if(entities.entities[0])
			{
				...
				request('https://finance.google.com/finance?output=json&q='+stockSymbol, function (error, res, body) {
                            ...
                            if (!error && res.statusCode == 200) {
							    ...
							    // console.log('HEADERS: ' + JSON.stringify(res.headers));
                                 ...
                                 {
                                      console.log('The current stock price of '+stockdetails[0].t+' is '+stockdetails[0].l);
                                      session.send('The current stock price of '+stockdetails[0].t+' is '+stockdetails[0].l);
                                 }
                            }
                            else
                            {
                                  console.log('Sorry, please check the stock code'+res.statusCode);
